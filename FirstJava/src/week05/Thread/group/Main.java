...
public class Main {
    public static void main(String[] args) {
        Runnable task = () -> {
            while (!Thread.currentThread().isInterrupted()) { // Thread.currentThread() -> 현재 실행중인 쓰레드의 상태 -> 현재 실행중인 쓰레드가 interruppted(대기상태)가 아닐때까지 무한루프 (5초 동안 무한루프)
                try {
                    Thread.sleep(1000);
            ...
            System.out.println(Thread.currentThread().getName() + " Interrupted"); // 반복문이 끝나면 실행
        };
